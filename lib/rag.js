...
export async function processQuery(message, history = [], summary = "") {
  ...
  // 4. Handle memory follow-ups
  if (classification.category === "memory_followup") {
    const recentHistory = trimHistory(history);
    if (!recentHistory || recentHistory.length === 0) {
      return {
        reply: "I don't have enough earlier conversation in this session yet.",
        classification,
        sources: [],
        memoryUpdate: null
      };
    ...
    const memoryPrompt = `You are HunterBot. The user is asking what you were talking about recently.
Look at the recent conversation history below and summarize what the user asked and what you discussed.
Keep it brief, friendly, and direct. Only use the provided history. DO NOT guess.`;
  ...
  if (classification.category === "greeting") {
    return {
      reply: "Hello! I'm HunterBot, your expert assistant for all things Hunter x Hunter. What can I help you with today?",
      classification,
      sources: [],
      memoryUpdate: null,
    };
  ...
  // 8. RAG retrieval for HxH queries
  ...
  // 11. Check if summary needs update
  let newSummary = null;
